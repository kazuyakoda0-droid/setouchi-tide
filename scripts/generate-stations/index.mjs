...
async function main() {
  ...
  const safe = [];
  let boundaryExcluded = 0, tooFarExcluded = 0, freshwaterExcluded = 0;
  for (const c of deduped) {
    if (isFreshwaterFishingPond(c)) { freshwaterExcluded++; continue; }
  ...
  console.log(`  残存: ${safe.length}件 / 除外(潮汐境界): ${boundaryExcluded}件 / 除外(観測点から遠すぎる): ${tooFarExcluded}件 / 除外(淡水釣り堀): ${freshwaterExcluded}件`);
