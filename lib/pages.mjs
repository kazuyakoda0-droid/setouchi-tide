...
import {
  ...
  shioBadge, weekTable, monthCalendar, stationList, coords, nowBox, forecastLinks, surfBlock,
} from './components.mjs';
...
import { PREF_NOTES, REGION_NOTES, profileText } from './area-notes.mjs';
...
function splitBlock(st, day, cel, neighbors, dayMs, dayNav, ymd, fc) {
  const pts = [
    mapStation(st, true),
...
// 潮・風・波を別々に探し回らず、「今日は何を確認すればよいか」を最初に
// 把握できる要約。釣果を断定する指数にはせず、判断材料をそのまま示す。
function tripBrief(day, fc) {
  const flow = movingWindows(day.levels).slice(0, 2)
    .map(w => `${w.fromStr}〜${w.toStr}`)
    .join(' / ') || '潮の動きが小さい日です';
  const weather = [];
  if (fc?.wave) weather.push(`波 ${fc.wave}`);
  if (fc?.wind) weather.push(`風 ${fc.wind}`);
  if (fc?.pop != null) weather.push(`降水 ${fc.pop}%`);
  return `<section class="trip-brief" aria-label="今日の海況チェック">
  <p class="trip-brief-title">今日の海況チェック</p>
  <dl>
    <div><dt>潮が動く目安</dt><dd>${esc(flow)}</dd></div>
    <div><dt>気象・海象</dt><dd>${esc(weather.join('　') || '気象予報は未発表です')}</dd></div>
  </dl>
  <p>推算潮位と予報をまとめた確認用です。安全判断は現地の情報も併用してください。</p>
</section>`;
}
...
export function stationPage(ctx) {
  const { st, day, cel, ymd, dayMs, weekRows, neighbors, fc, noindex, dayHref, dayNav } = ctx;
  const p = pref(st.pref);
...
<article>
  ...
  <p class="station-meta">${coords(st)}<span class="badge ${stationMarkerClass(st)}">${stationMarkerLabel(st)}</span>
    <button type="button" class="fav-toggle" data-favorite-toggle aria-pressed="false"><span aria-hidden="true">☆</span> お気に入りに追加</button>${shioBadge(cel.shio)}
  ...
  ${surfBlock(st, day, cel, fc, true)}

  ${disclosure('今日の海況を詳しく', '潮の動き・風・波・降水', tripBrief(day, fc))}

  ${disclosure('潮がよく動く時間帯', '潮位変化が大きい時間',
  ...
  ${disclosure('周辺地点・出典', '近くの地点とデータについて', relatedBlock(st, neighbors) + fishingLinksBlock(st) + deviationNote(st) + srcNote(st))}
  ${ad('header')}
  ${ad('graph')}
  ${ad('footer')}
</article>`;
...
<article>
  ...
  ${extremeTable(day)}
  ${surfBlock(st, day, cel, fc, isToday)}
  ${ad('header')}
...
// 都道府県・地方の潮汐プロファイル。文章も数値も lib/tide-profile.mjs が
// 実測した値から組み立てる。地域ごとに必ず違う内容になる。
function profileSection(name, note, pf) {
  if (!pf) return '';
  return section(`${name}の潮汐`, 'TIDE PROFILE', `
    <div class="prose">${profileText(name, note, pf, attr(paths.guide('kanmansa-chiikisa')))}</div>
    <div class="tw"><table class="prefsum">
      <thead><tr><th>指標</th><th>値</th></tr></thead>
      <tbody>
        <tr><th scope="row">平均の干満差</th><td>${pf.avg} cm</td></tr>
        <tr><th scope="row">大潮のころ</th><td>約 ${pf.spring} cm</td></tr>
        <tr><th scope="row">小潮のころ</th><td>約 ${pf.neap} cm</td></tr>
        <tr><th scope="row">年間で最も大きい干満差</th><td>${pf.max} cm（${esc(pf.maxStation)}）</td></tr>
        <tr><th scope="row">満潮が1日1回だけの日</th><td>年間の ${pf.diurnalPct}%</td></tr>
      </tbody>
    </table></div>
    <p class="note">気象庁の年次潮位表から、この地域の観測点${pf.n}地点ぶんを通年で集計した値です。</p>`);
}
...
export function prefPage(ctx) {
  const { p, r, rows, ymd, dateJa, profile } = ctx;
...
<article>
  ...
  ${ad('header')}
  ${profileSection(p.name, PREF_NOTES[p.id], profile)}
...
export function regionPage(ctx) {
  const { r, prefs, count, allStations, profile, prefProfiles } = ctx;
...
<article>
  ...
  ${profileSection(`${r.name}地方`, REGION_NOTES[r.id], profile)}
...
<article>
  ...
  ${section('特徴', 'FEATURES', `<ul class="feat">
    ...
    <li>潮位は<b>気象庁の公式推算値</b>をそのまま表示。${SITE.LEAN ? '掲載しているのは気象庁の潮位観測点そのものだけです。' : '公式観測点のない地点は近似であることを地点ごとに明示しています。'}</li>
  </ul>`)}
  ${ad('graph')}
