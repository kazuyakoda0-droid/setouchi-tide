...
function splitBlock(st, day, cel, neighbors, dayMs, dayNav) {
  const pts = [
    mapStation(st, true),
...
<article>
  <header class="st-hd">
    ...
    <p class="meta">${coords(st)}<span class="badge ${stationMarkerClass(st)}">${stationMarkerLabel(st)}</span>
      <button type="button" class="fav-toggle" data-favorite-toggle aria-pressed="false"><span aria-hidden="true">☆</span> お気に入りに追加</button></p>
  </header>
  ...
  <div class="daybar">
    ...
    ${shioBadge(cel.shio)}
    ${dayHref ? `<a class="detail" href="${attr(dayHref)}">この日を詳しく</a>` : ''}
  </div>

  ${splitBlock(st, day, cel, neighbors, dayMs, dayNav)}
