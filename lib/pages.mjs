...
<article>
  ...
  ${lead(`${dateJa}（${WD[cel.wd]}）の${withPref(st)}の潮汐は、満潮が${s.h}、干潮が${s.l}です。`
    + `潮回りは${cel.shio}、干満差は${day.range}cmです。${flowText(movingWindows(day.levels))}`)}
...
<article>
  ...
  ${lead(`${dateJa}（${WD[cel.wd]}）の${withPref(st)}の潮汐は、満潮が${s.h}、干潮が${s.l}です。`
    + `潮回りは${cel.shio}、干満差は${day.range}cm、月齢は${cel.age.toFixed(1)}です。${flowText(ws)}`)}
...
function stationLd(st, day, cel, ymd, dateModified) {
  return {
    ...
    publisher: { '@type': 'Organization', name: SITE.NAME, url: abs.home() },
