...
function mapStation(st, current = false) {
  return { n: st.name, la: st.lat, lo: st.lon, h: paths.station(st), m: stationMarkerClass(st), c: current ? 1 : 0 };
}
...
function srcNote(st) {
  ...
  return `<p class="src-note">この地点には公式観測点がないため、最寄りの気象庁観測点<b>${esc(st.jmaName || st.jma)}</b>${dist}の推算値を参照しています。${adj}${qualityNote}${osm}</p>`;
}
...
function fishingSearchFor(st) {
  ...
  const near = prefStations(st.pref)
    ...
    .slice(0, 2)
    .map(s => s.name);
  const terms = [...new Set([st.name, ...near])];
...
${stationList(others.map(s => ({
    href: paths.station(s), name: s.name, kana: s.kana, official: !s.jmaAnchor, tier: stationMarkerClass(s),
  })), 'cols')}
  ...
  <div class="split pin">
    <div class="split-info">
      ...
      <div class="tw"><table class="prefsum">
        ...
        <tbody>${rows.map(x => `<tr>
          <th scope="row"><a href="${attr(paths.station(x.st))}"><span class="dot ${stationMarkerClass(x.st)}"></span>${esc(x.st.name)}</a></th>
...
<article>
  ...
  ${prefs.map(p => section(p.name, '', stationList(
    prefStations(p.id).map(s => ({ href: paths.station(s), name: s.name, kana: s.kana, official: !s.jmaAnchor, tier: stationMarkerClass(s) })), 'cols'),
