...
function recentOf(st) {
  return { n: st.name, h: paths.station(st), p: pref(st.pref).name };
}
...
function srcNote(st) {
  ...
  const osm = st.id.startsWith('n')
    ? `地点の位置情報は<a href="https://www.openstreetmap.org/copyright" rel="noopener" target="_blank">OpenStreetMap</a>のデータを参照しています。` : '';
  return `<p class="src-note">この地点には公式観測点がないため、最寄りの気象庁観測点<b>${esc(st.jmaName || st.jma)}</b>${dist}の推算値を参照しています。${adj}あくまで近似値です。${osm}</p>`;
}
...
function fishingSearchFor(st) {
  if (st.pref !== 'hiroshima') return null;
  const west = st.lon < 132.68;
  const terms = west
    ? [st.name, '広島湾', '江田島', '呉', '宮島']
    : [st.name, '竹原', '大崎上島', '三原', '尾道'];
  const area = west ? '広島湾・江田島・呉' : '竹原・大崎上島・三原・尾道';
  const query = `(${terms.join(' OR ')}) (釣果 OR 釣り OR アジング OR メバリング) lang:ja -filter:replies`;
  return { href: `https://x.com/search?q=${encodeURIComponent(query)}&f=live`, label: `Xで${area}の釣果を検索` };
}
...
<p class="note">釣果情報は外部サイト・SNS検索によるものです。検索語は海域単位で設定し、地域の投稿を拾いやすくしています。安全確認や漁業権・立入制限は必ず現地の表示と関係者の指示に従ってください。</p>`);
}
...
export function dayPage(ctx) {
  const { st, day, cel, ymd, dayMs, prev, next, isToday, monthDays, fc, dateModified } = ctx;
  const p = pref(st.pref);
  const s = extremeSummary(day);
  const d = new Date(dayMs);
...
export function monthPage(ctx) {
  const { st, ym, cells, prev, next, months, stats } = ctx;
  const p = pref(st.pref);
...
<article>
  ...
  ${lead(`${dateJa}の${p.name}沿岸${rows.length}地点の潮見表です。公式観測点${official}地点、近似地点${rows.length - official}地点を掲載しています。`)}
  ...
  <div class="split pin">
    ...
    <div class="split-map">
      ...
      <p class="note">点をクリックするとその地点の潮見表に移ります。●は気象庁の公式観測点、▲は最寄りの公式観測値を表示する近似地点です。</p>`)}
    </div>
  </div>
  ${ad('header')}
...
<article>
  ...
  <div class="split wide-map">
    ...
    <div class="split-map">
      ...
      <p class="note">全${total}地点。●は気象庁の公式観測点、▲は最寄りの公式観測値を表示する近似地点です。</p>`)}
    </div>
  </div>
  ${ad('header')}
