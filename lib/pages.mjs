...
export function stationPage(ctx) {
  const { st, day, cel, ymd, dayMs, weekRows, neighbors } = ctx;
...
<article>
  ...
  ${section('気象・海象', 'WEATHER & SEA', weatherBlock(st, ymd))}
...
export function dayPage(ctx) {
  const { st, day, cel, ymd, dayMs, prev, next, isToday, monthDays } = ctx;
  const p = pref(st.pref);
  const s = extremeSummary(day);
  const d = new Date(dayMs);
...
<article>
  <header class="home-hd">
    <h1>${esc(SITE.NAME)}<span class="en">${esc(SITE.NAME_EN)}</span></h1>
  ...
  ${section('地方から選ぶ', 'AREAS', `<ul class="regions">${regions.map(r =>
    `<li><a href="${attr(paths.region(r.r))}"><span class="rn">${esc(r.r.name)}</span><span class="rc">${r.count}地点</span>
      <span class="rp">${esc(r.prefs.map(p => p.name).join('・'))}</span></a></li>`).join('')}</ul>`)}
  ${ad('graph')}
...
export function aboutPage() {
  ...
  const description = `${SITE.NAME}のデータ出典・計算方法・免責について。潮位は気象庁の公式推算値、気象・海象は Open-Meteo によります。`;
  const body = `
<article class="prose">
  <h1>このサイトについて</h1>
  <h2>データの出典</h2>
  ...
  <p>気象・海象（天気・気温・風・気圧・波高・水温）は <a href="${attr(SITE.OM_CREDIT_URL)}" rel="nofollow noopener" target="_blank">Open-Meteo</a> の forecast API / marine API によります。</p>
  <h2>公式観測点と近似地点</h2>
...
function leafletScripts() {
  return `<script src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js" crossorigin defer></script>`;
}
