...
// 潮位グラフ
// ---------------------------------------------------------------------
// viewBox は 920×420。横 100% で高さが追従するので、375px 幅の端末では
// 375×171 になる。16:5 のような横長にすると曲線が潰れて読めないため、
// スマホでも縦が確保できる比率にしてある。
// SVG 内の文字は縮小されると読めなくなるので、viewBox 単位で大きめ(15〜17)に
// 取ってある。正確な数値は直下の満干潮表と10分毎グリッドで読める。
export function tideGraph(day, cel) {
  const W = 920, H = 420, X0 = 58, X1 = 908, Y0 = 24, Y1 = 348;
  const lv = day.levels;
  ...
  for (const [a, b] of night) {
    g += `<rect x="${a.toFixed(1)}" y="${Y0}" width="${(b - a).toFixed(1)}" height="${Y1 - Y0}" fill="rgba(43,38,32,.055)"/>`;
  }
  for (let v = lo; v <= hi; v += step) {
    const yy = y(v).toFixed(1);
    g += `<line x1="${X0}" y1="${yy}" x2="${X1}" y2="${yy}" stroke="rgba(60,50,35,.10)" stroke-width="1"${v === lo ? '' : ' stroke-dasharray="3 4"'}/>`
      + `<text x="${X0 - 8}" y="${(y(v) + 5).toFixed(1)}" text-anchor="end" class="ax">${v}</text>`;
  }
  for (let h = 0; h <= 24; h += 3) {
    const xx = hx(h).toFixed(1);
    g += `<line x1="${xx}" y1="${Y1}" x2="${xx}" y2="${Y1 + 7}" stroke="rgba(60,50,35,.18)" stroke-width="1"/>`
...
export function extremeTable(day) {
  ...
  const rows = day.extremes.map(e => {
    const high = e.type === '満潮';
    return `<tr class="${high ? 'hi' : 'lo'}"><th scope="row">${high ? '▲ 満潮' : '▼ 干潮'}</th>`
...
export function sunMoon(cel) {
  ...
  return `<div class="sm">
  <div class="sm-col">
    <div class="sm-hd">太陽<span class="en">SUN</span></div>
    <dl>
...
export function weatherBlock(st, ymd, fc) {
  ...
  if (fc.tmax != null && fc.tmin != null && fc.tmax !== fc.tmin) {
    cells.push(['気温', `${fc.tmax} / ${fc.tmin}`, '℃']);
  } else if (fc.tmax != null) {
    cells.push(['最高気温', String(fc.tmax), '℃']);
  } else if (fc.tmin != null) {
    cells.push(['最低気温', String(fc.tmin), '℃']);
  } else {
    hasTemp = false;
  }
  if (fc.pop != null) cells.push(['降水確率', String(fc.pop), '%']);
  if (fc.wave) cells.push(['波', fc.wave, '']);

  const grid = cells.length
    ? `<div class="wx-grid">${cells.map(([k, v, u]) =>
      `<div><span class="k">${esc(k)}</span><span class="v">${esc(v)}`
      + `${u ? `<small>${esc(u)}</small>` : ''}</span></div>`).join('')}</div>`
    : '';

  const wind = fc.wind
    ? `<dl class="wx-txt"><dt>風</dt><dd>${esc(fc.wind)}</dd></dl>`
    : '';
  ...
  return `<div class="wx">
  ${fc.weather ? `<div class="wx-head"><span class="cond">${esc(fc.weather)}</span>${rel}</div>` : ''}
  ${grid}
  ${wind}
  <p class="note">${src}</p>
  ${windyLink(st)}
...
// ---------------------------------------------------------------------
export function weekTable(rows) {
  const body = rows.map(r => {
    ...
    return `<tr${r.today ? ' class="today"' : ''}>
      ...
      <td class="hi">${hi}</td>
      <td class="lo">${lo}</td>
      <td class="mn">${r.cel.age.toFixed(1)}</td>
    </tr>`;
  }).join('');
  return `<div class="tw"><table class="week">
  <thead><tr><th>日付</th><th>潮名</th><th>満潮 <small>cm</small></th><th>干潮 <small>cm</small></th><th>月齢</th></tr></thead>
  ...
  const body = cells.map(c => {
    ...
    const ex = c.day
      ? c.day.extremes.map(e =>
        `<li class="${e.type === '満潮' ? 'h' : 'l'}">${e.type === '満潮' ? '▲' : '▼'}${fmtHM(e.time)} <em>${Math.round(e.level)}</em></li>`).join('')
      : '';
