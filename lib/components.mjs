...
function icon(name, className = '') {
  const paths = {
    ...
    thunder: '<path d="M7 20h17a5 5 0 0 0 .3-10A8 8 0 0 0 9.1 8 6 6 0 0 0 7 20Z"/><path d="m17 21-4 6h4l-2 4 6-8h-4l2-2Z"/>',
...
// サーフモード
//
// 波向・周期・うねりは自前データに無いため、根拠の薄い「おすすめ指数」は
// 作らない。ユーザーがポイントごとの好みの潮位帯と上げ下げを保存し、
// その条件に入る時間を10分刻みの潮位からクライアント側で抽出する。
// 潮位系列はすでにSVGへ描画済みなので、ここで data-* に重複させない。
// ---------------------------------------------------------------------
export function surfBlock(st, day, cel, fc, isToday = false) {
  const margin = Math.round(day.range * .2);
  let defaultMin = Math.round((day.min + margin) / 5) * 5;
  let defaultMax = Math.round((day.max - margin) / 5) * 5;
  ...
  return `<section class="surf-card" id="surf-mode" data-surf data-surf-station="${attr(stationKey)}"
    data-surf-min="${defaultMin}" data-surf-max="${defaultMax}"
    data-sunrise="${cel.sunrise}" data-sunset="${cel.sunset}"${isToday ? ' data-surf-today' : ''}>
  <header class="surf-head">
    <div><span class="surf-kicker">SURF CHECK</span><h2>サーフモード</h2></div>
    <p>ポイントに合う潮位帯を保存して、入水候補を絞り込みます。</p>
  </header>
  <div class="surf-layout">
    <div class="surf-settings" aria-label="サーフモードの条件">
      <label><span>好みの潮位</span><span class="surf-range">
        <input type="number" data-surf-min-input value="${defaultMin}" min="-300" max="500" step="5" inputmode="numeric" aria-label="好みの最低潮位">〜
        <input type="number" data-surf-max-input value="${defaultMax}" min="-300" max="500" step="5" inputmode="numeric" aria-label="好みの最高潮位"><small>cm</small>
      </span></label>
      <label><span>潮の向き</span><select data-surf-direction aria-label="好みの潮の向き">
        <option value="both">上げ・下げ両方</option><option value="up">上げ潮</option><option value="down">下げ潮</option>
      </select></label>
      <label class="surf-daylight"><input type="checkbox" data-surf-daylight checked> 日の出から日の入まで</label>
      <p class="surf-saved" data-surf-saved aria-live="polite">この地点の設定として端末内に保存されます。</p>
    </div>
    <div class="surf-result">
      ${isToday ? `<p class="surf-now"><span>現在</span><b data-surf-now>計算中</b></p>` : ''}
      <h3>入水候補</h3>
      <ul data-surf-windows><li>条件に合う時間を計算しています。</li></ul>
      <p class="surf-result-note" data-surf-result-note></p>
    </div>
  </div>
  <dl class="surf-sea">
  ...
  <p class="surf-foot">波は広い予報区域の日単位の値です。波向・周期・うねりと現地情報は
    <a href="${attr(windyUrl(st))}" rel="nofollow noopener" target="_blank">Windyで確認</a>してください。候補時間は安全や波質を保証するものではありません。</p>
</section>`;
}
...
export function forecastLinks(st, fc) {
  ...
  return `<nav class="fclinks" aria-label="この地点の予報">`
