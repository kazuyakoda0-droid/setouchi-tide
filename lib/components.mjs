...
// 潮位グラフ
// ---------------------------------------------------------------------
// viewBox は 920×684。横 100% で高さが追従するので、375px 幅の端末でも
// 375×279 と、山谷の曲線が判読できる高さを確保できる。
//
...
// ラベル3行分（最大文字で約4em ≒ 120単位）が上下に必要なので、潮位の
// 上下余白はグラフ高の 21% ずつを最低ラインとして確保する。
export function tideGraph(day, cel, nav = null) {
  const W = 920, H = 684, X0 = 90, X1 = 894, Y0 = 56, Y1 = 628;
  const lv = day.levels;
  ...
  let lo = Math.floor((day.min - span * 0.21) / step) * step;
  let hi = Math.ceil((day.max + span * 0.21) / step) * step;
  for (let n = 0; n < 12; n++) {
    const range = hi - lo;
    if ((day.min - lo) / range < 0.21) lo -= step;
    else if ((hi - day.max) / range < 0.21) hi += step;
    else break;
  }
